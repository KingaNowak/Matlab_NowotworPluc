...
selpath =imageDatastore('C:\Users\kinga\Desktop\pom_program\to\LCTSC\LCTSC-Test-S2-101\08-17-2000-4DCT THORAX-84012\211.000000-Ave-IP10 0102030405060708090-67473','FileExtensions',{'.DCM','.dcm'}, 'ReadFcn',@dicomread);
...
kryterium=1;                % kryterium przynale¿noœci do obszaru
                            ...
                            %       punktu startowego
max_dJ=0.09;                 % maksymalna ró¿nica intensywnoœci 
...
for d=1:length(centroidXbie)
    for i=n+1:liczba
        ...
        if zdjecie(centroidYbie(d,1),centroidXbie(d,1))<=0.15
            ...
            P=max(pomocnicza(:));
            wy1=P>zdjecie(centroidYbie(d,1),centroidXbie(d,1));
            wy2=P>=0.15;
            if wy1==1 && wy2==1
                [wp,kp]=find(pomocnicza==P);
                centpom=strpom(kp).obraz;
